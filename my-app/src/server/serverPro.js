    ...
    app.get('/api/get_view/get_product:view',(req,res)=>{
        ...
        project_db.query(`select * from ${req.params.view} order by ranking asc`,(err,data)=>{
            if(!err){//오류없을때
                // console.log(data)
    ...
    app.get('/api/get/get_product:product_id&:view',(req,res)=>{
        // console.log("13Line",req.params)
        project_db.query(`select * from ${req.params.view} where id="${req.params.product_id}"`,(err,data)=>{
            if(!err){//오류없을때
                // console.log(data)
                res.send({Product:data})
                //res.send({hello:'hello response'})
    ...
    app.get('/api/get/get_review:product_id',(req,res)=>{ //해당 제품 리뷰 검색
        project_db.query(`select * from ${req.params.product_id}_review order by good_rec desc Limit 3`,(err,data)=>{
    ...
    app.get('/api/get/user_info_img:name',(req,res)=>{ //유저 기본이미지 가져오기
        project_db.query(`select profile_img from user_info where name="${req.params.name}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // console.log(data)
                res.send({img_src:data})
                //res.send({hello:'hello response'})
                //response.send
                //서버의 응답인데,App로 넘어간다.
            }else{//오류가있으면
    ...
    app.get('/api/get_review/get_review_all:product_id',(req,res)=>{ //해당 제품 리뷰 검색
        ...
        project_db.query(`select * from ${req.params.product_id}_review`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // console.log(data)
                res.send({Review:data})
    ...
    app.post('/api/get_review_person/get_review',(req,res)=>{ //해당 제품 리뷰 검색
        ...
        project_db.query(`select * from ${req.body.pro_id}_review where name="${req.body.writer}" and title="${req.body.title}" and time="${req.body.create_time}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // console.log(data)
                res.send({Review:data})
    ...
    app.get('/api/check/user_login:id&:pw',(req,res)=>{ //해당 제품 리뷰 검색
        ...
        project_db.query(`SELECT * FROM user_info WHERE id="${req.params.id}" AND passwd="${req.params.pw}"`,(err,data)=>{
            if(!err){//오류없을때
        ...
        // project_db.query(`SELECT * FROM users WHERE id="${req.params.id}" AND password="${req.params.pw}"`,(err,data)=>{
        //     if(!err){//오류없을때
        //         res.send({User_info:data})
        //     }else{//오류가있으면
        //         console.log(err)
    ...
    app.put('/api/put/modify_summary:id&:summary&:name&:title',(req,res)=>{ //해당 제품 리뷰 검색
        ...
        project_db.query(`update ${req.params.id}_review set summary="${req.params.summary}" where name="${req.params.name}" and title="${req.params.title}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // console.log(data)
                // res.send({User_info:data})
    ...
    app.get('/api/get_represent/get_product',(req,res)=>{
        ...
        project_db.query(`select * from represent`,(err,data)=>{
            if(!err){//오류없을때
                // console.log(data)
                res.send({data:data})
                //res.send({hello:'hello response'})
    ...
    app.post('/api/post_/create_review',(req,res)=>{ //리뷰 create , db 에 추가
        ...
        //console.log(`insert into ${id}_review (title,summary,date,inquiry,good_rec,bad_rec,name,time,review_img,review_img2,review_img3,review_img4,review_img5) values("${title}","${summary}","${date}",0,0,0,"${name_}","${time}","${img_src1}","${img_src2}","${img_src3}","${img_src4}","${img_src5}")`);
        project_db.query(`insert into ${id}_review (title,summary,date,inquiry,good_rec,bad_rec,name,time,review_img,review_img2,review_img3,review_img4,review_img5) values("${title}","${summary}","${date}",0,0,0,"${name_}","${time}","${img_src1}","${img_src2}","${img_src3}","${img_src4}","${img_src5}")`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
            }else{//오류가있으면
                console.log(err)
        ...
        project_db.query(`create table ${id}_comment_${name_}_${replace_date}_${time}(name varchar(50) , date varchar(30) , good_rec int , bad_rec int , summary varchar(1000), time varchar(50)) `,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
            }else{//오류가있으면
                console.log(err)
    ...
    app.delete('/api/delete_review',(req,res)=>{ // 게시판 db 삭제 및 댓글 db 삭제
        ...
        project_db.query(`DELETE FROM ${req.body.id}_review WHERE name="${req.body.name}" and summary="${req.body.summary}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
            }else{//오류가있으면
                console.log(err)
        ...
        project_db.query(`drop table ${req.body.id}_comment_${req.body.name}_${req.body.replace_title}_${req.body.replace_date}`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
            }else{//오류가있으면
                console.log(err)
    ...
    app.get('/api/get_comment:id&:title&:replace_title&:name&:date&:time',(req,res)=>{ // 날짜 넘겨받아야함 어떻게?
        ...
        project_db.query(`select * from ${req.params.id}_comment_${req.params.name}_${req.params.date}_${req.params.time}`,(err,data)=>{
            if(!err){//오류없을때
                res.send({Product:data})
            }else{//오류가있으면
                console.log(err)
    ...
    app.post('/api/insert_comment',(req,res)=>{ 
        ...
        project_db.query(`insert into ${pro_id}_comment_${writer}_${db_date}_${db_time} values("${writer}","${value_date}",0,0,"${summary}","${value_time}")`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
    ...
    app.delete('/api/delete/del_comment',(req,res)=>{ 
        ...
        project_db.query(`DELETE FROM ${req.body.pro_id}_comment_${req.body.writer}_${req.body.review_date}_${req.body.review_create_time} WHERE name="${req.body.writer}" and time="${req.body.comment_time}"`,(err,data)=>{
            if(!err){//오류없을때
                res.send({Update:"True"})
            }else{//오류가있으면
                console.log(err)
    ...
    app.get('/api/get_image/basic_image',(req,res)=>{
        // console.log(req.params)
        project_db.query(`select * from basic_image`,(err,data)=>{
            if(!err){//오류없을때
                // console.log(data)
                res.send({data:data})
                //res.send({hello:'hello response'})
    ...
    app.post('/api/update_user_img',(req,res)=>{ 
        ...
        project_db.query(`UPDATE user_info SET profile_img = "${img_src}" WHERE name = "${user_name}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // res.send({Update:"True"})
            }else{//오류가있으면
    ...
    app.put('/api/put/update_userinfo',(req,res)=>{ 
        ...
        project_db.query(`select * from user_info where name="${req.body.user_name}" and id="${req.body.user_id}" and passwd="${req.body.user_pw}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                res.send({data:data})
            }else{//오류가있으면
    ...
    app.put('/api/put/update_user_info_id',(req,res)=>{ 
        ...
        project_db.query(`UPDATE user_info SET id = "${req.body.update_id}" WHERE name = "${req.body.user_name}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // res.send({data:data})
            }else{//오류가있으면
    ...
    app.put('/api/put/select_user_password',(req,res)=>{ 
        project_db.query(`select * from user_info where name="${req.body.user_name}" and id="${req.body.user_id}" and passwd="${req.body.present_input_pw}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                res.send({data:data})
            }else{//오류가있으면
    ...
    app.put('/api/put/update_user_password',(req,res)=>{ 
        ...
        project_db.query(`UPDATE user_info SET passwd = "${req.body.update_input_pw}" WHERE name = "${req.body.user_name}" and id="${req.body.user_id}" and passwd="${req.body.present_input_pw}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                res.send({data:data})
            }else{//오류가있으면
    ...
    app.get('/api/get/user_sub_info:name',(req,res)=>{
        // console.log(req.params)
        project_db.query(`select email,phone_number,real_name,gender,name from user_info where name="${req.params.name}"`,(err,data)=>{
            if(!err){//오류없을때
                // console.log(data)
                res.send({data:data})
                //res.send({hello:'hello response'})
    ...
    app.get('/api/get/pro_name_review:id&:view',(req,res)=>{
        ...
        project_db.query(`select name from ${req.params.view} where id=${req.params.id}`,(err,data)=>{
            if(!err){//오류없을때
                // console.log(data)
                res.send({Product:data})
                //res.send({hello:'hello response'})
    ...
    app.put('/api/put/put_rec',(req,res)=>{ //해당 제품 리뷰 검색
        ...
        project_db.query(`update ${user_id}_review set good_rec = good_rec+1  where name="${user_name}" and time="${user_time}" and title="${user_title}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // res.send({data:data})
            }else{//오류가있으면
    ...
    app.put('/api/put/update_inpuiry',(req,res)=>{ //해당 제품 리뷰 검색
        user_id = req.body.id
        user_name = req.body.name
        user_time = req.body.time
        user_title = req.body.title
        // console.log(`update ${user_id}_review set inquiry = inquiry+1  where name="${user_name}" and time="${user_time}" and title="${user_title}"`)
        
        project_db.query(`update ${user_id}_review set inquiry = inquiry+1  where name="${user_name}" and time="${user_time}" and title="${user_title}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // res.send({data:data})
            }else{//오류가있으면
    ...
    app.post('/api/update_review_img',(req,res)=>{ //해당 제품 리뷰 검색
        ...
        project_db.query(`update ${user_id}_review set review_img = "${user_img_src1}" , review_img2 = "${user_img_src2}", review_img3 = "${user_img_src3}" , review_img4 = "${user_img_src4}", review_img5 = "${user_img_src5}"  where name="${user_name}" and time="${user_time}" and title="${user_title}"`,(err,data)=>{
            // console.log("/api/get/get_review:product_id")
            if(!err){//오류없을때
                // console.log(data)
                // res.send({Review:data})
